...
exports.resumableUploadFromFile = void 0;
...
const fsOpen = (0, util_1.promisify)(fs.open);
const fsRead = (0, util_1.promisify)(fs.read);
const fsClose = (0, util_1.promisify)(fs.close);
/**
 * 计算安全的分片大小
 */
function calculateSafePartSize(totalSize, expectPartSize) {
    const MIN_PART_SIZE = 5 * 1024 * 1024; // 5MB
    const MAX_PART_NUMBER = 10000;
    let partSize = expectPartSize || constants_1.MinChunkSize;
    // 确保分片大小 >= 5MB（最后一片除外）
 ...
 */
function generatePartTasks(totalSize, partSize, isLargeFile = false) {
    const tasks = [];
    for (let i = 0;; ++i) {
 ...
 * 断点续传上传（文件路径方式）
 */
async function resumableUploadFromFile(params) {
    var _a, _b, _c;
    const { spaceName, filePath, fileType, fileName, fileExtension, sessionKey, host, oid, auth, partSize: expectPartSize, maxConcurrency = 4, checkpoint, onProgress, onUploadEvent, cancelToken, initUploadPart, queryUploadParts, uploadPart, uploadMergePart, } = params;
    // 获取文件信息
    const fileStat = await fsStat(filePath);
    const fileSize = fileStat.size;
    // 大文件认定方式：只要文件大小 > MinChunkSize（需要分片上传），就认为是大文件
    // 这样 PartNumber 会从 1 开始，与 chunkUpload 的逻辑保持一致
    const isLargeFile = fileSize > constants_1.MinChunkSize;
    // 计算实际分片大小
    const actualPartSize = calculateSafePartSize(fileSize, expectPartSize || constants_1.MinChunkSize);
    // 加载 checkpoint
    const checkpointInfo = await (0, checkpoint_1.loadCheckpoint)(checkpoint);
    ...
    let checkpointFilePath = checkpointInfo.filePath ||
    ...
    if (!checkpointFilePath && checkpointRecord) {
        checkpointFilePath = (0, checkpoint_1.getDefaultCheckpointFilePath)(checkpointRecord.space_name, checkpointRecord.file_path || filePath);
    }
    // 校验 checkpoint
    const isValidCheckpoint = await (0, checkpoint_1.validateCheckpoint)(checkpointRecord, filePath, fileSize, actualPartSize);
    ...
    if (checkpointRecord) {
        ...
        if (queryUploadParts) {
            try {
                const serverParts = await queryUploadParts(host, oid, auth, uploadID, isLargeFile, fileName);
                if (serverParts.length > 0) {
                    // 1) 使用 allTasks 映射服务端 partList，得到“已完成”的 partsInfo
                    // 如果服务端不返回 crc32，优先从本地 checkpoint 查找
                    const localPartsMap = new Map();
                    ...
                    partsInfo = serverParts
                        ...
                        if (!crc32Value) {
                            ...
                            if (localPart === null || localPart === void 0 ? void 0 : localPart.crc32) {
                    ...
                    if (checkpointFilePath) {
                        await (0, checkpoint_1.saveCheckpoint)(checkpointFilePath, checkpointRecord);
                    }
                }
                else {
                ...
                if (checkpointFilePath) {
                    await (0, checkpoint_1.removeCheckpoint)(checkpointFilePath);
                }
                checkpointRecord = undefined;
            }
        }
        ...
        if (checkpointRecord && shouldTriggerInitEvent) {
            onUploadEvent === null || onUploadEvent === void 0 ? void 0 : onUploadEvent({
                type: types_1.VodUploadEventType.InitUploadPartSucceed,
                spaceName,
                uploadId: uploadID,
            });
    ...
    if (!checkpointRecord) {
        ...
        try {
            ...
            onUploadEvent === null || onUploadEvent === void 0 ? void 0 : onUploadEvent({
                type: types_1.VodUploadEventType.InitUploadPartFailed,
                spaceName,
                uploadId: "",
    ...
    // 过滤已上传的分片
    const uploadedPartSet = new Set(partsInfo.filter((p) => p.is_completed).map((p) => p.part_number));
    ...
    if (checkpointRecord && partsInfo.length < allTasks.length) {
        ...
        // 更新 checkpoint 记录
        checkpointRecord.parts_info = Array.from(partsInfoMap.values()).sort((a, b) => a.part_number - b.part_number);
        // 保存更新后的 checkpoint
        if (checkpointFilePath) {
            await (0, checkpoint_1.saveCheckpoint)(checkpointFilePath, checkpointRecord);
        }
    ...
    const checkSumList = new Array(allTasks.length);
    // 填充已上传分片的 CRC32
    ...
    // 用于跟踪正在上传的分片，防止重复上传
    const uploadingPartSet = new Set();
    const updateCheckpoint = async (part) => {
        // 如果已经完成合并，不再更新进度
        if (mergeCompleted) {
            return;
        }
        ...
        if (checkpointFilePath) {
            // 不等待文件写入完成，避免阻塞并发上传
            (0, checkpoint_1.saveCheckpoint)(checkpointFilePath, checkpointRecord).catch((err) => {
        ...
        if (currentPercentFloor > lastPercentFloor && !mergeCompleted) {
            ...
            onProgress === null || onProgress === void 0 ? void 0 : onProgress(percent, checkpointRecord);
        }
    };
    try {
        await Promise.all(remainingTasks.map((task) => limit(async () => {
            var _a, _b;
            // 检查取消
            if (cancelToken === null || cancelToken === void 0 ? void 0 : cancelToken.reason) {
                onUploadEvent === null || onUploadEvent === void 0 ? void 0 : onUploadEvent({
                    type: types_1.VodUploadEventType.UploadPartAborted,
                    spaceName,
                    uploadId: uploadID,
                });
            ...
            if (existingPart === null || existingPart === void 0 ? void 0 : existingPart.is_completed) {
            ...
            try {
                ...
                try {
                    ...
                    if (finalCheck === null || finalCheck === void 0 ? void 0 : finalCheck.is_completed) {
                        // 更新 checkSumList
                        const taskIndex = allTasks.findIndex((t) => t.partNumber === task.partNumber);
                    ...
                    let crc32Value;
                    try {
                        crc32Value = await uploadPart(host, oid, auth, uploadID, task.partNumber, buffer, isLargeFile);
                    }
                    catch (err) {
                        // 上传失败，从正在上传集合中移除
                        ...
                        if (((_a = err.message) === null || _a === void 0 ? void 0 : _a.includes("404")) || ((_b = err.response) === null || _b === void 0 ? void 0 : _b.status) === 404) {
                            const errorMsg = `UploadID 已过期或无效。` +
                                `UploadID: ${uploadID}, ` +
                    ...
                    onUploadEvent === null || onUploadEvent === void 0 ? void 0 : onUploadEvent({
                        type: types_1.VodUploadEventType.UploadPartSucceed,
                        spaceName,
                        uploadId: uploadID,
                        partInfo: {
                            partNumber: task.partNumber,
        ...
        checkSumList.forEach((crc, index) => {
            ...
            if (!crc) {
                ...
                const partNumber = (_a = allTasks[index]) === null || _a === void 0 ? void 0 : _a.partNumber;
        ...
        try {
            ...
            const err = mergeErr instanceof Error
                ? mergeErr
                : new Error(mergeErr !== undefined && mergeErr !== null ? String(mergeErr) : "合并失败");
            onUploadEvent === null || onUploadEvent === void 0 ? void 0 : onUploadEvent({
                type: types_1.VodUploadEventType.UploadMergePartFailed,
                spaceName,
                uploadId: uploadID,
                err,
            });
            // 如果是 400 错误且提示 invalid merge parts，可能是 uploadID 过期或格式不对
            if (((_a = err.message) === null || _a === void 0 ? void 0 : _a.includes("InvalidMergeParts")) ||
                ((_b = err.message) === null || _b === void 0 ? void 0 : _b.includes("invalid merge parts")) ||
                ((_c = mergeErr === null || mergeErr === void 0 ? void 0 : mergeErr.response) === null || _c === void 0 ? void 0 : _c.status) === 400) {
                const wrapErr = new Error(`合并失败: uploadID 可能已过期或无效。` +
                    `UploadID: ${uploadID}, ` +
                    ...
                    `错误详情: ${err.message}`);
                wrapErr.isMergeFailure = true;
                throw wrapErr;
            }
            err.isMergeFailure = true;
            throw err;
        }
        // 标记合并已完成，防止后续的进度回调
        mergeCompleted = true;
        onUploadEvent === null || onUploadEvent === void 0 ? void 0 : onUploadEvent({
            type: types_1.VodUploadEventType.UploadMergePartSucceed,
            spaceName,
            uploadId: uploadID,
        });
        ...
        if (checkpointFilePath) {
            ...
            const defaultPath = (0, checkpoint_1.getDefaultCheckpointFilePath)(checkpointRecord.space_name, checkpointRecord.file_path || filePath);
            await (0, checkpoint_1.removeCheckpoint)(defaultPath);
        }
    }
    catch (err) {
        const isAborted = err instanceof Error && err.message === "上传已取消";
        const isMergeFailure = Boolean(err && typeof err === "object" && err.isMergeFailure);
        if (!isAborted && !isMergeFailure) {
            onUploadEvent === null || onUploadEvent === void 0 ? void 0 : onUploadEvent({
                type: types_1.VodUploadEventType.UploadPartFailed,
                spaceName,
                uploadId: uploadID,
                err: err instanceof Error ? err : new Error(String(err)),
            });
...
exports.resumableUploadFromFile = resumableUploadFromFile;
