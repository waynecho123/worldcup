...
let liveInfo = {};
let matchDetails = {};
let resolvedOdds = {}; // fixture ID → match ID resolved odds

// Load match details + odds
wx.request({
  ...
  success: function(res) {
    if (res.data) {
      ...
      // After details loaded, load odds and resolve fixture IDs
      wx.request({
        url: 'https://waynecho123.github.io/worldcup/odds.json',
        timeout: 10000,
        success: function(r2) {
          if (!r2.data) return;
          var fixToMid = {};
          Object.keys(matchDetails).forEach(function(mid) {
            ...
            if (d && d.fixtureId) fixToMid[d.fixtureId] = mid;
          });
          Object.keys(r2.data).forEach(function(k) {
            var o = r2.data[k];
            if (!o || !o.h) return;
            var mid = fixToMid[k] || k;
            resolvedOdds[mid] = o;
          });
...
// Dynamic lineup: use last match actual lineup, minus suspended players
function getDynamicLineup(teamId) {
  var team = data.TEAMS.find(function(t){return t.id === teamId});
  if (!team) return null;

  // Detect suspensions from match events
  var suspended = [];
  var yellows = {};
  Object.keys(matchDetails).forEach(function(mid) {
    var d = matchDetails[mid];
    if (!d || !d.events) return;
    if (d.homeTeam !== team.name && d.awayTeam !== team.name) return;
    (d.events||[]).forEach(function(e) {
      if (e.type === 'Card' && e.detail === 'Red Card') suspended.push(e.player);
      if (e.type === 'Card' && e.detail === 'Yellow Card') {
        yellows[e.player] = (yellows[e.player]||0) + 1;
        if (yellows[e.player] >= 2) suspended.push(e.player);
      }
    });
  });
  suspended = [...new Set(suspended)];

  // Find last match actual lineup
  var latestLu = null, latestDate = '';
  Object.keys(matchDetails).forEach(function(mid) {
    var d = matchDetails[mid];
    if (!d || !d.lineups) return;
    var lu = null;
    if (d.homeTeam === team.name && d.lineups.home) lu = d.lineups.home;
    if (d.awayTeam === team.name && d.lineups.away) lu = d.lineups.away;
    if (lu && lu.startXI && d.date > latestDate && lu.formation) {
      latestDate = d.date;
      latestLu = lu;
    }
  });

  if (latestLu && latestLu.startXI) {
    var formation = latestLu.formation;
    var players = latestLu.startXI.filter(function(p) {
      return suspended.indexOf(p.player.name) < 0;
    }).map(function(p) { return p.player.name; });
    return { formation: formation, players: players };
  }

  // Fallback to hardcoded
  var hc = data.STARTING_XI[teamId];
  if (!hc) return null;
  var all = [];
  if (hc.g) (Array.isArray(hc.g)?hc.g:[hc.g]).forEach(function(n){all.push(n);});
  if (hc.d) hc.d.forEach(function(n){all.push(n);});
  if (hc.m) hc.m.forEach(function(n){all.push(n);});
  if (hc.am) hc.am.forEach(function(n){all.push(n);});
  if (hc.fwd) hc.fwd.forEach(function(n){all.push(n);});
  if (hc.st) (Array.isArray(hc.st)?hc.st:[hc.st]).forEach(function(n){all.push(n);});
  return { formation: hc.f, players: all };
}

// Format lineup for display
function fmtLineup(lu) {
  if (!lu) return '暂无';
  if (Array.isArray(lu.players)) {
    return lu.formation + ' ' + lu.players.join(' · ');
  }
  var all = [];
  if (lu.g) (Array.isArray(lu.g)?lu.g:[lu.g]).forEach(function(n){all.push(n);});
  if (lu.d) lu.d.forEach(function(n){all.push(n);});
  if (lu.m) lu.m.forEach(function(n){all.push(n);});
  if (lu.am) lu.am.forEach(function(n){all.push(n);});
  if (lu.fwd) lu.fwd.forEach(function(n){all.push(n);});
  if (lu.st) (Array.isArray(lu.st)?lu.st:[lu.st]).forEach(function(n){all.push(n);});
  return lu.f + ' ' + all.join(' · ');
}
...
Page({
  ...
  buildDetail(matchId) {
    ...
    const hLu = fmtLineup(getDynamicLineup(m.home));
    const aLu = fmtLineup(getDynamicLineup(m.away));
  ...
  generatePostMatchReview(m, ht, at, pred, actual, apiInfo) {
    ...
    // 9. Match events & lineups from API-Sports
    var detail = matchDetails[matchId || m.id];
    if (detail && detail.events && detail.events.length > 0) {
      var goals = detail.events.filter(function(e){return e.type === 'Goal'});
      var cards = detail.events.filter(function(e){return e.type === 'Card'});
      var subs = detail.events.filter(function(e){return e.type === 'subst'});
      var eventLines = [];
      if (goals.length > 0) {
        var goalText = goals.map(function(g){
          var pen = g.detail === 'Penalty' ? '(点)' : '';
          return g.time + '\' ' + g.player + pen + (g.assist ? '(助:' + g.assist + ')' : '');
        }).join('，');
        eventLines.push('⚽ 进球：' + goalText);
      }
      if (cards.length > 0) {
        var cardText = cards.map(function(c){return c.time + '\' ' + c.player + ' ' + (c.detail||'');}).join('，');
        eventLines.push('🟨 黄牌：' + cardText);
      }
      if (subs.length > 0) {
        var subText = subs.slice(0,6).map(function(s){return s.time + '\' ' + s.player + '↑';}).join('，');
        eventLines.push('🔄 换人：' + subText);
      }
      if (eventLines.length > 0) parts.push('📺 【赛事过程】' + eventLines.join(' | '));
    }
    if (detail && detail.lineups) {
      var fmtLu = function(lu) {
        if (!lu || !lu.startXI) return '';
        var players = lu.startXI.map(function(p){return p.player.number + '.' + p.player.name;}).join(' ');
        return (lu.formation || '') + ' [' + players + ']';
      };
      var hLu = fmtLu(detail.lineups.home);
      var aLu = fmtLu(detail.lineups.away);
      if (hLu || aLu) {
        parts.push('📋 【实际首发】' + ht.cn + ' ' + hLu + ' | ' + at.cn + ' ' + aLu);
      }
    }

    // 10. Root cause analysis (when prediction was wrong)
    if (!outcomeCorrect) {
